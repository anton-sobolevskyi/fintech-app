...
export const lookupAccountByIban = onCall<LookupData>(async (request) => {
  ...
  const snap =
    await db.collection("accounts").where("iban", "==", iban).limit(1).get();
  ...
  if (account.userId) {
    const userSnap =
      await db.collection("users").doc(account.userId).get();
...
export const transferFunds = onCall<TransferData>(async (request) => {
  ...
  const destSnap =
    await db.collection("accounts").where("iban", "==", destIban).limit(1).get();
  ...
  await db.runTransaction(async (tx) => {
    const [sourceSnap, dest] =
      await Promise.all([tx.get(sourceRef), tx.get(destRef)]);
